...
public class PoiSearch {
	 ...
	 * 查询POI。
	 * 
	 * 如果此时矩形查询（SearchBound）已定义，则搜索范围为该矩形内的符合条件的POI，否则
	 * 如果此时查询条件（Query）中的行政区划代码已定义，则搜索该城市（地区）内的所有符合条件的POI。
	 * 如上述两个条件均未定义，则搜索范围为全国。
	 * 为了性能及结果的清晰，强烈建议定义一个范围：矩形或行政区划代码均可。整个查询的语义为查找符合查询关键字，符合查询类型编码组合及在指定范围内的所有POI。
	...
	public PoiResult searchPOI() throws SearchException {
		...
		QueryInternal queryInternal = new QueryInternal(this.query, this.bound);
		PoiSearchServerHandler handler = new PoiSearchServerHandler(context,
				queryInternal, AppInfo.getProxy(context), null);
		return handler.GetData();
	}

//    private boolean isEmpty(){
//        if(AppInfo.IsEmptyOrNullString(getQuery().getLocation().toString())||AppInfo.IsEmptyOrNullString(getQuery().getQueryString())
//                && AppInfo.IsEmptyOrNullString(getQuery())){
//
//        }
//
//    }
	...
	/**
	 * 查询POI异步接口。
	 * 如果此时矩形查询（SearchBound）已定义，则搜索范围为该矩形内的符合条件的POI，否则如果此时查询条件（Query）中的行政区划代码已定义，
	 * 则搜索该城市（地区）内的所有符合条件的POI,如上述两个条件均未定义，则搜索范围为全国。
	 * 建议定义一个范围 ：矩形或行政区划代码均可。整个查询的语义为查找符合查询关键字，符合查询类型编码组合及在指定范围内的指定POI。
	 * 查询POI的结果，结果是分页的，每页最多50 个。 此方法为异步处理。
	 */
     ...
     * @throws SearchException  此接口在出现问题的情况下，会抛出SearchException。
     */
    public List<PoiItem> searchPOIId(java.lang.String poiID,java.lang.String adcode) throws SearchException {
        PoiIdSearchServerHandler handler = new PoiIdSearchServerHandler(context, poiID, AppInfo.getProxy(context), null,adcode);
        return handler.GetData();
    }
     ...
     * @param poiID  poi的ID。
     */
    public void searchPOIIdAsyn(final java.lang.String poiID,final java.lang.String adcode){
        new Thread(new Runnable() {
            public void run() {
                ...
                try {
                    ...
                    localMessage.obj = locale;
                    locale.listener = PoiSearch.this.OnPoiSearchListener;
                    locale.result = PoiSearch.this.searchPOIId(poiID,adcode);
                } catch (SearchException localSearchException) {
                    localMessage.arg2 = localSearchException.getErrorCode();
                } finally {
                    if (null != PoiSearch.this.handler)
	...
	public static class Query {
		...
		private int scope = 1;// 详细程度，默认基本信息
		private String region;// 区域名称
		private int radius;// 查询半径
		private LatLonPoint location;// 周边中心坐标
		private int pageNum = 1;
		private int pageSize = 10;
        private String category;//poi类型。
		 ...
		 *            查询字符串。不能为空。
		 * @param city
		 *            待查询城市（地区）的城市名称（中文）。必设参数，不能为空。
		 */
		public Query(String query, String city){
			this(query, null, city);
		}
		...
		private boolean isValidQuery() {
			return !AppInfo.IsEmptyOrNullString(query) && !AppInfo.IsEmptyOrNullString(region);
		}
		 ...
		 * 设置结果的详细程度。
		 *
		 * @param mScope 结果的详细程度。 1为基本信息，2为详细信息。
		 */
...
//			centerToRect(center, AppInfo.MeterToE6(radiusInMeters),
//					AppInfo.MeterToE6(radiusInMeters));
        ...
        /**
		 * 根据给定的参数来构造PoiSearch.SearchBound的新对象。
		 * 如果超出中国边界或者 lowerLeft >= upperRight，则抛出IllegalArgumentException异常。
		 * @param lowerLeft
		 *            矩形的左下角。
		 * @param upperRight
		 *            矩形的右上角。
...
//        }
		...
		/**
		 * 根据给定的参数来构造PoiSearch.SearchBound的新对象，默认由近到远排序。
		 * 如果超出中国边界或者radiusInMeters<=0，则抛出IllegalArgumentException异常。
		 * 
		 * @param center
		 *            该范围的中心点。
		 * @param radiusInMeters
