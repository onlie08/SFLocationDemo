...
import com.sfmap.api.mapcore.IUiSettingsDelegate;
...
public final class UiSettings
{
  ...
  public void setCompassEnabled(boolean enabled)
  {
    ...
    /**
     * 设置定位按钮是否显示。这个按钮可以让当前可视区域移动到以用户所在位置为中心的地图上。在这里定位按钮显示同时还要保证定位层处于显示状态， 定位按钮才会显示。 默认的定位按钮为显示。
     * @param enabled - true 为显示定位按钮；false 禁止显示定位按钮。
     */
  public void setMyLocationButtonEnabled(boolean enabled)
  {
    try
    {
      this.uiSettingsDelegate.setMyLocationButtonEnabled(enabled);
    }
    catch (RemoteException e)
    {
      throw new RuntimeRemoteException(e);
