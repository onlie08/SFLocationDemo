...
public class Gradient
{
  ...
  private Gradient(int[] paramArrayOfInt, float[] paramArrayOfFloat, int paramInt)
  {
    try
    {
      if ((paramArrayOfInt == null) || (paramArrayOfFloat == null)) {
        throw new MapException("colors and startPoints should not be null");
      }
      if (paramArrayOfInt.length != paramArrayOfFloat.length) {
        throw new MapException("colors and startPoints should be same length");
      }
      if (paramArrayOfInt.length == 0) {
        throw new MapException("No colors have been defined");
      }
      for (int i = 1; i < paramArrayOfFloat.length; i++) {
        if (paramArrayOfFloat[i] <= paramArrayOfFloat[(i - 1)]) {
          throw new MapException("startPoints should be in increasing order");
        }
      }
      this.a = paramInt;
    ...
    {
      this.d = false;
      Log.e("MapSDK", localMapException.getErrorMessage());
  ...
  protected int[] generateColorMap(double transparent)
  {
    ...
    if (transparent != 1.0D) {
      ...
      {
        int k = arrayOfInt[j];
        arrayOfInt[j] = Color.argb((int)(Color.alpha(k) * transparent),
