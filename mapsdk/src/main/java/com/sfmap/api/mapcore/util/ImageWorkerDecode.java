...
import com.sfmap.api.mapcore.TileOverlayDelegateImp;
...
public abstract class ImageWorkerDecode {
						  ...
						  TileOverlayDelegateImp.TileCoordinate tile) // a->loadImage
	{
		...
		if (this.mImageCache != null) {
			...
			localBitmap = this.mImageCache.getBitmapFromMemCache(localObject.toString());
		}
		if (localBitmap != null) {
			...
			task.executeOnExecutor(AsyncTaskDecode.THREAD_POOL_EXECUTOR,
					new Boolean[] { Boolean.valueOf(paramBoolean) });
		}
	}

	public void addImageCache(ImageCacheDecode.ImageCacheParams cacheParams) // a ->addImageCache
	{
		this.mImageCacheParams = cacheParams;
		this.mImageCache = ImageCacheDecode.getInstance(this.mImageCacheParams);
		new CacheAsyncTask().execute(new Object[] { Integer.valueOf(MESSAGE_INIT_DISK_CACHE) });
	}
	...
	public class BitmapWorkerTask // a
		...
		{
			this.tileReference = new WeakReference<>(parama);
		}
		...
		protected Bitmap doInBackground(Boolean... paramVarArgs) {
			...
			TileOverlayDelegateImp.TileCoordinate tile = this.tileReference
					.get();
			if (tile == null) {
				return null;
			}
			...
			String tileKey = localStringBuilder.toString();
			...
			if ((mImageCache != null)
					...
					&& (! mExitTasksEarly)) {
				tileBitmap = mImageCache.getBitmapFromDiskCache(tileKey);
			}
			...
			if (tileBitmap != null) {
				if ( mImageCache != null) {
					 mImageCache.put(tileKey, tileBitmap);
				}
			}
