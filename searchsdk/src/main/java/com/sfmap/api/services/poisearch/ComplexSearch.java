...
public class ComplexSearch {
     ...
     *                        异常信息。
     */
    ComplexSearchResult searchData() throws SearchException {
        ComplexSearchServerHanler handler = new ComplexSearchServerHanler(
                context, getQuery(), AppInfo.getProxy(context), null);
    ...
    public static class Query {
        ...
        private int radius;
        /**
         * 每页记录数，最大值为50。 默认值为10，超过50则按照50处理。
         */
         ...
         * 构造一个综合搜索的Query对象。
         * @param query
         *             待查询的关键字，query和type不可同时存在,query不为空时,type必须置空;
         *             当datasource为bus时,type和query可以同时为空,可以进行公交地铁站的分类查询。
         * @param type
         *            poi的类型,query和type不可同时存在,type不为空时,query必须置空;
         *            当datasource为bus时,type和query可以同时为空,可以进行公交地铁站的分类查询。
         * @param datasource
        ...
        /**
         * 返回每页记录数，最大值为50。 默认值为10，超过50则按照50处理。
         * @return 每页记录数，最大值为50。 默认值为10，超过50则按照50处理。
         */
        ...
        /**
         * 设置每页记录数，最大值为50。 默认值为10，超过50则按照50处理。
         * @param page_size
         *                 每页记录数，最大值为50。 默认值为10，超过50则按照50处理。
         */
