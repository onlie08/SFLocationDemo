  ...
  implements Parcelable, Cloneable
{
  ...
  private static DecimalFormat a = new DecimalFormat("0.0000000000000000", new DecimalFormatSymbols(Locale.US));
  ...
  public LatLng(double latitude, double longitude, boolean isCheck)
  {
    if (isCheck)
    {
      ...
      if ((latitude < -90.0D) || (latitude > 90.0D)) {
        try
        {
          throw new MapException("非法坐标值");
        }
        catch (MapException ex)
        {
          ex.printStackTrace();
          Log.d("MapSDK", ex.getErrorMessage());
        }
      }
