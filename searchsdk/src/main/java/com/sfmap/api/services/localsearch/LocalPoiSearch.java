...
public class LocalPoiSearch {
     ...
     * @throws SearchException 搜索异常信息。
     */
    public LocalPoiSearch(Context context, String snPath, String dataPath)throws SearchException {
        if(context != null){
     ...
     */
    public LocalPoiResult searchPOI(LocalPoiSearch.Query query) throws SearchException {
        if (query==null){
            throw new SearchException(SearchException.ERROR_NULL_PARAMETER);
        }
    ...
    private LocalPoiResult searchName(LocalPoiSearch.Query query) throws SearchException {
        int resetRes = 0;
        int count = 0;
        ...
        LocalSearchAPI search = LocalSearchAPI.getInstance();
            ...
            if(resetRes == -1){
                poiResult.setMessage(SearchException.ERROR_LOCAL_AUTH_FAILURE);
                throw new SearchException(SearchException.ERROR_LOCAL_AUTH_FAILURE);
            }
            if(resetRes == 1){
    ...
    private LocalPoiResult searchFristLetter(LocalPoiSearch.Query query) throws SearchException {
        int resetRes = 0;
        int count = 0;
        List<SearchResultInfo> resList = new ArrayList<SearchResultInfo>();
        LocalPoiResult poiResult = new LocalPoiResult(query,resList);
