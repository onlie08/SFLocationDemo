...
public class SDKLogHandler  extends BasicLogHandler  implements Thread.UncaughtExceptionHandler
{
  ...
  public static synchronized SDKLogHandler a(Context paramContext, SDKInfo sdkInfo)   throws IMMapCoreException
  {
    ...
    try
    {
      if (BasicLogHandler.logHandler == null) {
        BasicLogHandler.logHandler = new SDKLogHandler(paramContext, sdkInfo);
      } else {
        BasicLogHandler.logHandler.c = false;
      }
      BasicLogHandler.logHandler.a(paramContext, sdkInfo, BasicLogHandler.logHandler.c);
    }
    catch (Throwable throwable)
    {
      throwable.printStackTrace();
    }
    return (SDKLogHandler) BasicLogHandler.logHandler;
  }
  ...
  public static void normalLog(int customid,String content,String className,String methodName){
    if(BasicLogHandler.logHandler!=null){
      BasicLogHandler.logHandler.normalLog(customid,content,3,className,methodName);
    }
  }
  public static synchronized void colseLogThread()
  {
    ...
    try
    {
      if ((BasicLogHandler.logHandler != null) && (Thread.getDefaultUncaughtExceptionHandler() == BasicLogHandler.logHandler)
              && (BasicLogHandler.logHandler.caughtExceptionThread != null)) {
  ...
  private void setExceptionHandler()
  {
    try
    {
      ...
      else
      {
        ...
        String packageName = "com." + Util.CONFIT_KEY_COMPANY + ".api";
