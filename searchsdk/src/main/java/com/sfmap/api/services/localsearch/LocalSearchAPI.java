...
public class LocalSearchAPI {
     ...
     * @param adLevel 行政区划编码的等级。请参见 {@link ADCodeLevel }。
     * @param searchType 搜索类型，是名称关键字搜索还是地址关键字搜索。请参加 {@link SearchType }。
     ...
     * @param adCode 行政区划编码。注：SDK暂不对行政区划编码的有效性进行检验，如输入“110300”不存在的行政区划编码
     * @param adLevel 行政区划编码的等级。请参见 {@link ADCodeLevel }。
     * @param searchType 搜索类型，是名称首拼搜索还是地址首拼搜索。请参加 {@link SearchType }。
     * @return 重置是否成功。 0代表失败， 1代表成功,-1表示鉴权失败。
     */
