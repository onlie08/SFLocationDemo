  ...
  extends RouteOverlay
{
  ...
  private PolylineOptions busLineOptions;
     ...
     * @param map - 地图对象。
     * @param path - 驾车路线规划的一个方案。详见搜索服务模块的路径查询包（com.sfmap.api.services.route）中的类 {@link DrivePath}。
     * @param start - 起点。详见搜索服务模块的核心基础包（com.sfmap.api.services.core）中的类 {@link LatLonPoint}。
     * @param end - 终点。详见搜索服务模块的核心基础包（com.sfmap.api.services.core）中的类 {@link LatLonPoint}。
     */
  public DrivingRouteOverlay(Context context, MapController map, DrivePath path, LatLonPoint start, LatLonPoint end)
  {
     ...
     * @param start - 起点。详见搜索服务模块的核心基础包（com.sfmap.api.services.core）中的类 {@link LatLonPoint}。
     * @param end - 终点。详见搜索服务模块的核心基础包（com.sfmap.api.services.core）中的类 {@link LatLonPoint}。
     * @param throughPointList  - 途经点列表，详见搜索服务模块的核心基础包（com.sfmap.api.services.core）中的类{@link LatLonPoint}。
     */
