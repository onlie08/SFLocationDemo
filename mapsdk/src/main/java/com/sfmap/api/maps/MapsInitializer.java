...
/**
 * 在获取地图之前某些功能想要使用，可以使用这个类初始化地图API。 由于一些类比如BitmapDescriptorFactory、CameraUpdateFactory需要被初始化，就必须调用类MapsInitializer。 如果您正在使用MapFragment或MapView，并且调用getMap()获得非空的地图，您不需要关注这个类。
 */
public final class MapsInitializer
{
  ...
  private static final String MAPSDK_VERSION = "1.7.0";
  private static int coodSystem = COOD_SYSTEM_900913;
    /**
     * 初始化地图API,为使用它包含的类做准备。 在这些类中，您如果正在使用MapFragment或MapView，并且调用getMap()获得非空的地图，您可以不需要调用这个方法。
