...
public class RouteSearch {
	...
	public WalkRouteResult calculateWalkRoute(
			RouteSearch.WalkRouteQuery walkQuery) throws SearchException {
		WalkSearchServerHandler handle = new WalkSearchServerHandler(
				this.context, walkQuery, AppInfo.getProxy(this.context), null);
	...
	public DriveRouteResult calculateDriveRoute(
			RouteSearch.DriveRouteQuery driveQuery) throws SearchException {
		DriveSearchServerHandler handle = new DriveSearchServerHandler(
				this.context, driveQuery, AppInfo.getProxy(this.context), null);
		return handle.GetData();
	}
	...
	public BusRouteResult calculateBusRoute(RouteSearch.BusRouteQuery busQuery)
		...
		BusSearchServerHandler handle = new BusSearchServerHandler(
				this.context, busQuery, AppInfo.getProxy(this.context), null);
		return handle.GetData();
	}
	...
	public static class BusRouteQuery implements Parcelable, Cloneable {
		...
		/**
		 * 序列化实现。
		 * @param parcel Parcel对象实例。
		 */
		public BusRouteQuery(Parcel parcel) {
			this.fromAndTo = ((RouteSearch.FromAndTo) parcel
					.readParcelable(RouteSearch.FromAndTo.class
							.getClassLoader()));
			this.tactics = parcel.readInt();
		...
		public BusRouteQuery(FromAndTo fromAndTo) {
			...
			if (AppInfo.isNull(fromAndTo))
				try {
					throw new SearchException(
							SearchException.ERROR_NULL_PARAMETER);
		 ...
		 * 返回坐标类型。
		 * 
		 * @return 坐标类型：参数取值 ：gcj02. 为国测局加密坐标 ;wgs84.为gps设备获取坐标， 默认为：gcj02。
		 */
		 ...
		 * 设置坐标类型
		 * 
		 * @param coordType 参数取值 ：gcj02. 为国测局加密坐标 ;wgs84.为gps设备获取坐标， 默认为：gcj02。
		 *
		 */
	...
	public static class WalkRouteQuery implements Parcelable, Cloneable {
		 ...
		 * 设置坐标类型。
		 * 
		 * @param coordType 坐标类型，参数取值 ：gcj02. 为国测局加密坐标 ;wgs84.为gps设备获取坐标， 默认为：gcj02。
		 */
		public void setCoordType(String coordType) {
			this.coord_Type = coordType;
		}
		 ...
		 * 设置导航策略。
		 * 
		 * @param tactics 导航策略，参数取值 ：11.为最少时间，12.为最短路径， 默认为:11。
		 */
		public void setTactics(int tactics) {
			this.tactics = tactics;
		}
