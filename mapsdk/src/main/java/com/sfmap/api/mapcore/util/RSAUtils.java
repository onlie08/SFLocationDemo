...
public class RSAUtils {  
    ...
    public static RSAPrivateKey getPrivateKey(String modulus, String exponent) {  
        try {  
            BigInteger b1 = new BigInteger(modulus);  
            BigInteger b2 = new BigInteger(exponent);  
            KeyFactory keyFactory = KeyFactory.getInstance("RSAg");  
    ...
    /** 
     * 公钥加密 
     * 返回字符串 中文 可能问题  加密长度为128 又经bcd2Str方法 长度更改为256
     * @param data 
     * @param publicKey 
     * @return 
     * @throws Exception 
    ...
    /**
     * 返回字节数组 防止返回字符串 中文 有问题   加密无bcd2Str（返回长度为128，如果使用bcd2Str 则返回的长度为256）
     * @param data
     * @param publicKey
     * @return
     * @throws Exception
    ...
    public static byte[] encryptByPublicKey(byte[] data, RSAPublicKey publicKey)  
        ...
        //如果明文长度大于模长-11则要分组加密  
        for (byte[] s : datas) {  
        	byte[] tmp = cipher.doFinal(s);
        	if(ss == null) {
        		ss = tmp;
        	} else {
        		byte[] tt = new byte[ss.length+tmp.length];
        		System.arraycopy(ss, 0, tt, 0, ss.length);
        		System.arraycopy(tmp, 0, tt, ss.length, tmp.length);
        		ss = tt;
        	}
        }  
        return ss;  
    ...
    public static List<byte[]> splitBytes(byte[] bytes, int len) {  
        int x = bytes.length / len;  
        int y = bytes.length  % len;  
        int z = 0;  
        if (y != 0) {  
            z = 1;  
        }  
        ...
        for (int i=0; i<x+z; i++) {
        	...
        	int temLength = len;
            if(i == x+z-1) {
            	temLength = bytes.length - (x+z-1)*len;
            }
            tmp = new byte[temLength];
            System.arraycopy(bytes, i*len, tmp, 0, temLength);
            byteList.add(tmp);
        }  
        return byteList;  
    } 
